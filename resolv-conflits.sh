cat > fix-all-conflicts.sh << 'SCRIPT_END'
#!/bin/bash
set -e

echo "🔧 Resolvendo TODOS os conflitos..."
...
# Header.tsx (ARQUIVO COMPLETO NO SCRIPT ACIMA - copie o conteúdo)
...
# .env
cat > .env << 'EOF'
VITE_WHATSAPP_NUMBER=5511978986350
VITE_WA_CAMPAIGN=site_whatsapp
EOF

# Commit
git add src/ .env
if [ -d ".git/rebase-merge" ]; then
    git rebase --continue || git commit -m "fix: resolver conflitos" || true
else
    git commit -m "fix: resolver todos os conflitos"
fi
